...
public:
    ...
    // Methods to get/set register values
    uint16_t readHoldingRegister(uint16_t address);
    bool writeHoldingRegister(uint16_t address, uint16_t value);
    
    // Methods to update register map from sensor data
    void updateFromSensor(const Sensor& sensor);
